...
// sliderGapSize: The distance between each break on the slider itself. For example, for a slider with a range of 0 - 100 split into four sections, the sliderGapSize is 25. This function assumes your slider has a range starting at 0.
...
let sliderValueToPiecewiseValue = (sliderGapSize, breaks, input) => {
    ...
    return breaks[closestSliderBreak] + ((input - sliderBreaks[closestSliderBreak]) / sliderGapSize) * diffs[closestSliderBreak]
}
...
let piecewiseValueToSliderValue = (sliderGapSize, breaks, input) => {
    ...
    return sliderGapSize * ((input - breaks[closestBreak]) / diffs[closestBreak]) + closestBreak * sliderGapSize
}
